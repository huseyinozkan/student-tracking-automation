...
namespace MySoft.OgrenciTakip.UI.Win.UserControls.UserControl.TahakkukEditFormTable
{
    public partial class OdemeBilgileriTable : BaseTablo
    {
                ...
                switch (entity.OdemeTipi)
                {
                    case OdemeTipi.Cek:
                        ...
                        if (entity.BankaSubeId == null)
                        {
                            ...
                            tablo.SetColumnError(colBankaSubeAdi, "Banka Sube Seçimi Yapmalısınız");
                        }
        ...
        {
            if (tablo.DataRowCount == 0) return;
            if (Messages.SilMesaj("Odeme Bilgisi") != DialogResult.Yes) return;

            var entity = tablo.GetRow<OdemeBilgileriL>();
            if (entity.SonHareketId != null)
            {
                Messages.OdemeBelgesiSilinemezMesj(false);
                return;
            }
