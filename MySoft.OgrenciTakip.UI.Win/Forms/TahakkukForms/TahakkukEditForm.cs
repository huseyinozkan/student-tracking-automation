...
using MySoft.OgrenciTakip.UI.Win.UserControls.UserControl.TahakkukEditFormTable;
using System;
...
namespace MySoft.OgrenciTakip.UI.Win.Forms.TahakkukForms
{
    public partial class TahakkukEditForm : BaseEditForm
    {
        ...
        // Kod ile çağırılacak olan tablolar
        ...
        protected override void NesneyiKontrollereBagla()
        {
            ...
            txtRehber.Text = entity.TesvikAdi;
        ...
        //Hatali giris oldugunda kullanilmali.
        protected override bool BagliTabloHataliGirisKontrol()
        {
            ...
            if (hizmetBilgileriTable.HataliGiris()) // sürükle bırak yöntemi
            {
            ...
            if (odemeBilgileriTable.HataliGiris()) // sürükle bırak yöntemi
            {
            ...
            if (geriOdemeBilgileriTable.HataliGiris()) // sürükle bırak yöntemi
            {
        ...
        protected internal override void ButonEnabledDurum()
        {
            ...
            bool TableValueChanged()
            {
                ...
                if (hizmetBilgileriTable.TableValueChanged) return true; // sürükle bırak yöntemi
                if (indirimBilgileriTable.TableValueChanged) return true; // sürükle bırak yöntemi
                if (odemeBilgileriTable.TableValueChanged) return true; // sürükle bırak yöntemi
                if (geriOdemeBilgileriTable.TableValueChanged) return true; // sürükle bırak yöntemi
        ...
        protected override bool BagliTabloKaydet()
        {
            ...
            if (!hizmetBilgileriTable.Kaydet()) return false; // sürükle bırak yöntemi
            if (!indirimBilgileriTable.Kaydet()) return false; // sürükle bırak yöntemi
            if (!odemeBilgileriTable.Kaydet()) return false; // sürükle bırak yöntemi
            if (!geriOdemeBilgileriTable.Kaydet()) return false; // sürükle bırak yöntemi

            return true;
        }

        // ButtonEnableDurum ve BağlıTabloYukle de çağırılıyor
        ...
        // EntityInsert ve EntityUpdate'de Kullanılıyor
        ...
        {
            // Kod ile yuklenen tablolar yuklenmediyse yukleme islemi yapiliyor.
        ...
        protected override void Control_SelectedPageChanged(object sender, SelectedPageChangedEventArgs e)
        {
            ...
            {
                if (layoutControlGroupAileSinavBilgileri.Items.Count == 0)
                {
                    _aileBilgileriTable = new AileBilgileriTable().AddTable(this);
                    layoutControlGroupAileSinavBilgileri.LayoutControlInsert(_aileBilgileriTable, 0, 0, 0, 0);
                    _aileBilgileriTable.Yukle();

                    _sinavBilgileriTable = new SinavBilgileriTable().AddTable(this);
                    layoutControlGroupAileSinavBilgileri.LayoutControlInsert(_sinavBilgileriTable, 1, 0, 0, 0);
                    _sinavBilgileriTable.Yukle();
                }
            ...
            // Sürükle bırak yöntemi kullanıldığı için kod ile ekleme yapmıyoruz
